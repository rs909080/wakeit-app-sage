...
async function doSetAlarm() {
        ...
        // F1: Check for offline members (missing device token)
        ...
        if (members) {
          ...
          const offlineNames = members
            .filter(m => m.user_id !== uid && !hasToken.has(m.user_id))
        ...
        // Seed alarm_wake_status for every group member
        const { data: groupMembers } = await db.from('group_members')
          .select('user_id').eq('group_id', group.id);
        if (groupMembers?.length) {
          await db.from('alarm_wake_status').insert(
            groupMembers.map(m => ({ alarm_id: alarm.id, user_id: m.user_id, status: 'pending' }))
          );
        }
